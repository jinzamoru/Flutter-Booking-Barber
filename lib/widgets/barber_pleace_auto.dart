...
import 'barber_select_google_map.dart';


class BarberPlaceAutocompleteWidget extends StatefulWidget {
  final BarberInfo barberlocation;

  const BarberPlaceAutocompleteWidget({super.key, required this.barberlocation});

  @override
  _BarberPlaceAutocompleteWidgetState createState() =>
      _BarberPlaceAutocompleteWidgetState();
}

class _BarberPlaceAutocompleteWidgetState
    extends State<BarberPlaceAutocompleteWidget> {
  ...
  void initState() {
    ...
    super.initState();
    _controller.text = model.barberNamelocation;
    selectedLatitude = model.barberLatitude;
    selectedLongitude = model.barberLongitude;
  }
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Container(
        ...
        child: MaterialButton(
          onPressed: () async {
           final barbermodel = BarberInfo(barberId: model.barberId, 
           barberFirstName: model.barberFirstName,
            barberLastName: model.barberLastName, 
            barberPhone: model.barberPhone, barberEmail: model.barberEmail, 
            barberPassword: "", 
            barberIDCard: model.barberIDCard, 
            barberNamelocation: _controller.text, 
            barberLatitude: selectedLatitude, barberLongitude: selectedLongitude, );
            Navigator.push(
                context,
                MaterialPageRoute(
                    builder: (context) => BarberMap(
                          barberlocation: barbermodel,
                        )));
          },
